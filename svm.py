...
def polynomial_kernel(X_train, X_test, y_train, y_test):
    ...
    poly_kernel_svm_clf = SVC(kernel='poly', degree=8)
...
def sigmoid_kernel(X_train, X_test, y_train, y_test):
    # TODO
    # Trains, predicts and evaluates the model
    sig_kernel_svm_clf = SVC(kernel='sigmoid')

    sig_kernel_svm_clf.fit(X_train, y_train)
    y_pred = sig_kernel_svm_clf.predict(X_test)
